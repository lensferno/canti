...
type ServiceInstallSurvey struct {
	Username  string
	Password  string
	Reconnect bool
	Method    string
...
func NewServiceInstallSurvey() *ServiceInstallSurvey {
	return &ServiceInstallSurvey{}
}

func (s *ServiceInstallSurvey) Ask() error {
	question := []*survey.Question{
		{
			...
			Name: "config",
			Prompt: &survey.Input{
				Message: "快好了...最后，配置文件需要保存到哪？（默认：当前目录）",
				Default: "./",
			},
		},
	}
